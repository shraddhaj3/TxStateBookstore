		...
		<div id="list-subject" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="courseName" title="${message(code: 'subject.courseName.label', default: 'Course Name')}" />
					
						<g:sortableColumn property="courseNumber" title="${message(code: 'subject.courseNumber.label', default: 'Course Number')}" />
				...
				<g:each in="${subjectInstanceList}" status="i" var="subjectInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${subjectInstance.id}">${fieldValue(bean: subjectInstance, field: "courseName")}</g:link></td>
					
						<td>${fieldValue(bean: subjectInstance, field: "courseNumber")}</td>
